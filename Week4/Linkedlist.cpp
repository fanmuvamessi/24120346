...
bool removeHead(List *&L)
{
    ...
    if(temp==nullptr)
        return false;
    if(L->p_tail==L->p_head)
    {
        delete temp;
        temp = nullptr;
        L->p_tail=L->p_head=nullptr;
        return true;
    }
...
void removeBefore(List *&L, int val)
{
    ...
    while(p->p_next->p_next->key != val)
    {
        p=p->p_next;
        if(p->p_next->p_next == nullptr)
            return;
    }
    NODE* temp = p->p_next;
...
bool addPos(List *&L, int data, int pos)
{
    ...
    if(p->p_next==nullptr)
    {
        p->p_next=ans;
        L->p_tail=ans;
        //cout<<L->p_head->key<<L->p_head->p_next->key<<L->p_tail->key<<"adasd";
        return true;
    }
    ans->p_next=p->p_next;
    p->p_next=ans;
...
bool addBefore(List *&L, int data, int val)
{
    ...
    {
        p=p->p_next;
        if(p->p_next==nullptr) return false;
    }
